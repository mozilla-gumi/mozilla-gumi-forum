...
#------------------------------------------
my $ver = "Child Tree v8.92 (modified)";# (ƒcƒŠ[®Œf¦”Â)
#------------------------------------------
# Copyright(C) ‚è‚ã‚¤‚¢‚¿
# E-Mail:ryu@cj-c.com
...
#--------------------------------------------------------
    if ($htype eq "T") {
        ...
        $Border = 1;
        $Twidth = 90;
        if ($hanyo eq "randam") {$icon = "ƒAƒCƒRƒ“<br>ƒ‰ƒ“ƒ_ƒ€"; }
    } elsif ($htype eq "TR") {
        if ($ResNo eq "e") {
            $ResNo = "e‹L–";
            $Twidth = 100;
        } else {
            $ResNo = "‚±‚ÌƒXƒŒƒbƒh’† $ResNo ”Ô–Ú‚Ì•ÔM";
            $Twidth = 90;
        }
        $Border = 0;
...
sub html2_ {
    ...
    if($cou){&con_;}
    print"<div class=\"Caption03l\">‘S $total ƒgƒsƒbƒN’† $Pg ` $Pg2 ‚ğ•\\¦</div>\n";
    $Plink="$Bl$Ble\n"; $a=0;
    for($i=0;$i<=$page_;$i++){
        $af=$page/($tpmax*$tab_m);
        if($i != 0){$Plink.=" ";}
        ...
        if($topok){$TP="<th width=\"13\%\">ƒgƒsƒbƒNì¬Ò</th>";}
        if($he_tp==0){$SK="<th width=\"13\%\">ÅI”­Œ¾Ò</th>";}
        if($end_f){$EE='<th width="5%">ó‹µ</th>';}
        $TableChange=0;
        foreach ($page .. $page_end) {
            ...
            if($TableChange==0){
                ...
                print"<th width=\"46\%\">ƒgƒsƒbƒNƒ^ƒCƒgƒ‹</th><th width=\"8\%\">‹L–”</th>$TP$SK<th width=\"15\%\">ÅIXV</th>$EE</tr>\n";
            }
            $TableChange++;
            if($i_mode){
                $File=0; $Size=0;
            ...
            $FL="<br>„¤<span class=\"ArtId\">#$namber</span>@[ì¬“ú:$date]";
    ...
    &allfooter("ƒgƒsƒbƒN$view");
    &foot_;
}
#--------------------------------------------------------------------------------------------------------------------
# [ƒRƒƒ“ƒgˆø—p]
# -> ƒgƒsƒbƒN/ƒXƒŒƒbƒh•\¦‚ÌÛ‚Ìˆø—pˆ—(comin_)
#
sub comin_{
    if ($sp==0) {$re = 1; }
    ...
    ($com, $com_) = split('\t', $com);
    if ($FORM{'In'} eq "") {
        $com = "¡No$nam‚É•ÔM($na‚³‚ñ‚Ì‹L–)<br>$co";
        $com =~ s/<br>/\n&gt; /g;
        $com =~ s/&gt; &gt; /&gt;&gt;/g;
    }
    $com =~ s/&nbsp;/ /g;
...
$fhy ='<h2><a name="F">‚±‚ÌƒgƒsƒbƒN‚É‘‚«‚±‚Ş</a></h2>';
if($KLOG){$fhy="";}
$total=@TOP;
...
$Plink.="</div>";
print"$Plink<br>";
if($Dk){print"($DkŒ‚Ìíœ‹L–‚ğ”ñ•\\¦)<br>";}
print"\n";
...
foreach $lines (@TREE) {
    ...
    $kiji_exist=1;
        ...
        print"\n<tr align=\"center\"><th>‘O‚Ì‹L–(Œ³‚É‚È‚Á‚½‹L–)</th>\n";
        print"<th>Ÿ‚Ì‹L–(‚±‚Ì‹L–‚Ì•ÔM)</th></tr>\n";
    }
...
if((!$name)||($name eq ' ')||($name eq '@')){$name=$noname;}
    ...
    if($nam eq $namber){$im="<strong class=\"Highlight\">";$im2="";$im3=" *Œ»İ‚Ì‹L–<\/strong>";$ii=1;}
    if($Keisen){$Tree.="$Sen";}
    else{
...
<li>$all_i ‚ğƒNƒŠƒbƒN‚·‚é‚Æ‚»‚ÌƒcƒŠ[‚ğˆêŠ‡‚Å•\\¦‚µ‚Ü‚·B</li>
</ul>$Henko<hr>
_HTML_
if($i_mode){&minf_("T");}
if($cou){&con_;}
print"<div class=\"Caption03l\">‘S $total ƒcƒŠ[’† $Pg ` $Pg2 ”Ô–Ú‚ğ•\\¦</div>\n";
...
print "<hr width=\"95%\">\n";
&allfooter("ƒcƒŠ[$a_max");
&foot_;
}
#--------------------------------------------------------------------------------------------------------------------
# [ƒcƒŠ[ˆêŠ‡•\¦]
# -> ƒcƒŠ[‚ÌŠÖ˜A‹L–‚ğ•\¦‚·‚é(all_)
#
sub all_ {
@TREE=();
open(DB,"$log");
...
if($cou){&con_;}
print<<"_ALLTOP_";
<div class="Caption03l">ƒcƒŠ[ˆêŠ‡•\\¦$IcCom</div>
...
<div class="Caption01c"><strong>‘Sƒy[ƒW</strong> / 
_FTOP_
...
if(@NEW){
    ...
    print"<div class=\"Caption01c\"><strong>‘Sƒy[ƒW</strong> / ";
    print"$Plink<br></div>\n";
}else{print"V’…‹L–‚Í‚ ‚è‚Ü‚¹‚ñB</div>\n";}
&foot_;
}
#--------------------------------------------------------------------------------------------------------------------
# [ƒƒO‘‚«‚±‚İˆ—]
...
sub wri_ {
    ...
    if ($FORM{"PV"} && ($FLAG == 0)) {
        ...
        if ($AgSg) {
            if ($FORM{"AgSg"}) {$HTML .= "‹L–ƒ\\[ƒg:ã‚°‚é(age)"; }
            else {$HTML .= "‹L–ƒ\\[ƒg:‰º‚°‚é(sage)"; }
        }
        print<<"_PV_";
<h2>ƒvƒŒƒrƒ…[</h2>
$HTML
...
#new_ = “Šeƒf[ƒ^A$lines = ƒƒOA@new=o—Íƒf[ƒ^
...
_ENDBOX_
            ...
            if ($o_mail) {
                ...
                $Mbox= <<_MAIL_;
<tr><td colspan="2">
 *ŠÖ˜A‚·‚éƒŒƒX‹L–‚ğƒ[ƒ‹‚ÅóM‚µ‚Ü‚·‚©? <select name="send">
<option value="1"$Y>‚Í‚¢
<option value="0">‚¢‚¢‚¦
</select> 
 ƒAƒhƒŒƒX <select name="pub">
<option value="0">”ñ•\\¦
<option value="1"$Pch>•\\¦
</select></td></tr>
_MAIL_
...
<tr><td><strong>E ƒ[ƒ‹</strong></td><td>
...
#                if ($FORM{'pass'} eq "$pass") {
            ...
            print "<tr><td><strong>ƒ^ƒCƒgƒ‹</strong></td><td>";
            ...
            print "<tr><td colspan=\"2\"><strong>ƒRƒƒ“ƒg</strong>";
            print "’Êíƒ‚[ƒh/<input type=\"radio\" name=\"pre\" value=\"0\" $T>";
            print "}•\\ƒ‚[ƒh/<input type=\"radio\" name=\"pre\" value=\"1\" $Z>";
            $com=$comment;
            &smile_decode($com);

            print '(“K“–‚É‰üs‚ğ“ü‚ê‚Ä‰º‚³‚¢)<br>' . "\n" . '<textarea name="comment" rows="15" cols="80" ';
            if ($BBFACE) {
...
</td></tr><tr><td colspan="2" align="right"><input type="submit" value=" •Ò W " >
_HTML_
            print "<input type=\"reset\" value=\"ƒŠƒZƒbƒg\"></td></tr></table></form></ul><hr width=\"95\%\">";
            if($i_mode){
                if($ico){
                    &size;
                    print<<"_DEL_";
...
_DEL_
                ...
                print<<"_DEL_";
<br>
1) ‘¾š‚ÌŠg’£q‚Í‰æ‘œ‚Æ‚µ‚Ä”F¯‚³‚ê‚Ü‚·B<br>
2) ‰æ‘œ‚Í‰Šúó‘Ô‚Åk¬ƒTƒCƒY$H2~$W2ƒsƒNƒZƒ‹ˆÈ‰º‚Å•\\¦‚³‚ê‚Ü‚·B<br>
3) “¯–¼ƒtƒ@ƒCƒ‹‚ª‚ ‚éA‚Ü‚½‚Íƒtƒ@ƒCƒ‹–¼‚ª•s“KØ‚Èê‡A<br>
@@ƒtƒ@ƒCƒ‹–¼‚ª©“®•ÏX‚³‚ê‚Ü‚·B<br>
4) ƒAƒbƒv‰Â”\\ƒtƒ@ƒCƒ‹ƒTƒCƒY‚Í1‰ñ<strong>$max_fs\KB</strong>(1KB=1024Bytes)‚Ü‚Å‚Å‚·B<br></ul>
<input type="hidden" name="mode" value="h_w"><input type="hidden" name="pass" value="$FORM{"pass"}">
...
#--------------------------------------------------------------------------------------------------------------------
# [ƒpƒXƒ[ƒh‰ğ“Ç]
# -> ƒpƒXƒ[ƒh‚ğˆÃ†‰»‚µƒ}ƒbƒ`ƒ“ƒO(cryma_)
#
sub cryma_ {
...
#            if($FORM{"pass"} eq $pass){$ok="m";}
        ...
        if($EStmp){
            &time_("");
            $EditCom="$date •ÒW";
            if($mo || $ok eq "m"){$EditCom.="(ŠÇ—Ò)";}else{$EditCom.="(“ŠeÒ)";}
            if($comment !~ /([0-9][0-9]):([0-9][0-9]):([0-9][0-9]) •ÒW/){$EditCom.="<br><br>";}else{$EditCom.="<br>";}
...
if(@E_ || @I_ || $FORM{'UP'}){
    if($mo && (@E_ || @I_)){&ent_;}
        ...
        if(@I_){$msg="<h3>ƒtƒ@ƒCƒ‹íœ</h3>"; $FORM{"del"}=$I_[0];}
...
#--------------------------------------------------------------------------------------------------------------------
# [ƒJƒEƒ“ƒgƒAƒbƒv]
# -> ‰ß‹ƒƒO”Ô†‚ÌƒJƒEƒ“ƒgƒAƒbƒv(log_up)
#
sub log_up {
    $n++;
...
if($Top_t){
    print "<li>$new_tŠÔˆÈ“à‚Éì¬‚³‚ê‚½ƒXƒŒƒbƒh‚Í $new_i ‚Å•\\¦‚³‚ê‚Ü‚·B</li>\n";
    print "<li>$new_tŠÔˆÈ“à‚ÉXV‚³‚ê‚½ƒXƒŒƒbƒh‚Í $up_i_ ‚Å•\\¦‚³‚ê‚Ü‚·B</li>\n";
}
...
if($Top_t){
    ...
    print"<strong><a name=\"list\">‹L–ƒŠƒXƒg</a></strong> ( )“à‚Ì”š‚Í•ÔM”</div>\n$List\n</div><br>\n";
}
$LinkNo="";
foreach ($page .. $page_end) {
    ($T,$nam,$date,$name,$email,$d_may,$comment,$url,
...
if($type){print"</div>\n";}
    ...
    if(@RES){
        ...
        print"<hr><div class=\"Caption01l\">‘S•ÔM $RC Œ’† $Pg ` $RC ”Ô–Ú‚ğ•\\¦</div><br>\n";
        $RC_=int($RC/$ResHy);
        ...
        if($RC){
#			if($Top_t){print"<hr><a href=\"#list\">¡‹L–ƒŠƒXƒg</a> /\n";}
            print"<hr><div class=\"Caption01r\">•ÔM—pƒXƒŒƒbƒh•\\¦\n";
            $a=0;
            for($i=0;$i<=$RC_;$i++){
...
print"<hr>\n";
&allfooter("ƒXƒŒƒbƒh$alk_su");
&foot_;
}
#--------------------------------------------------------------------------------------------------------------------
# [‰æ‘œ•æ“¾]
# -> ƒtƒ@ƒCƒ‹‚ª‰æ‘œ‚Ìê‡Aƒtƒ@ƒCƒ‹‚ğ“Ç‚İ‚ñ‚Å•‚ğæ“¾‚µ‚Ü‚·B‚»‚êˆÈŠO‚ÌƒAƒCƒRƒ“•\¦‚à‚¨‚±‚È‚¢‚Ü‚·(size)
...
<option value="2">‘S‹–‰ÂÏ‹L–ƒ`ƒFƒbƒN
<option value="0">ƒ`ƒFƒbƒN‚ğ‚Í‚¸‚·
_ENT_
print <<"_ENT_";
...
while ($line=<LOG>) {
    ...
    if($ico || ($Ent==0 && $mas_c==2)){
        if($i==0){
            ...
            print"<th>‹–‰Âƒ`ƒFƒbƒN</th><th>“ŠeÒî•ñ</th><th>ƒRƒƒ“ƒg</th><th>ƒtƒ@ƒCƒ‹î•ñ</th><th>ƒtƒ@ƒCƒ‹íœ</th></tr>\n";
        }
        $check="";	
        if($Ent){$eok="›"; if($FORM{"check"}==2){$check=" checked";}}
        else{$eok="<span class=\"red\">~</span>"; if($FORM{"check"}==1){$check=" checked";}}
        if($ty){$Re="($tyƒŒƒX)";}else{$Re="";}
if((!$name)||($name eq ' ')||($name eq '@')){$name=$noname;}
...
close(LOG);
if($TB){print"</table>";}
print "<br><input type=\"submit\" value=\"‹–‰Â/–¢‹–‰Â ƒtƒ@ƒCƒ‹íœ\"></form>\n";
&foot_;
}
#--------------------------------------------------------------------------------------------------------------------
# [•\¦Œ`®]
...
E‹L–’†‚Ì‰æ‘œ•\\¦Œ`®<select name="min">
<option value="0"$S>$W2~$H2ˆÈ‰º‚Ék¬
<option value="1"$S2>Œ´¡‘å
...
#--------------------------------------------------------------------------------------------------------------------
# [ƒAƒbƒvƒtƒ@ƒCƒ‹ˆê——]
# -> ƒAƒbƒv‚³‚ê‚½ƒtƒ@ƒCƒ‹‚ğˆê——‚Å•\¦‚µ‚Ü‚·(f_a_)
#
sub f_a_ {
&hed_("All Up File");
print <<"_ENT_";
...
sub check_ {
    ...
    if ($i_mode && $UP) {
        ...
        if (-e "$i_dir/$file") {
            $TIME = time;
            $file = "$TIME$TAIL";
            $Henko = "<h3>“¯–¼ƒtƒ@ƒCƒ‹‚ª‚ ‚Á‚½‚½‚ßA$file‚É•ÏX‚µ‚Ü‚µ‚½</h3>";
        } elsif ($file =~ /[^\w\-\.]/) {
            $TIME = time;
            $file = "$TIME$TAIL";
            $Henko = "<h3>ƒtƒ@ƒCƒ‹–¼‚ª•s“KØ‚¾‚Á‚½‚½‚ßA$file‚É•ÏX‚µ‚Ü‚µ‚½</h3>";
        }
        $MaxSize = $max_fs * 1024;
